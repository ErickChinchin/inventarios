	...
	public static function addView($servid,$field,$table){
		if(Session::getUID()!=""){
			...
			if(self::checkView($servid,$field,$table)==true){
				$con = Database::getCon();
					$sql = "insert into $table (viewer_id, $field,realip,created_at) values (".Session::getUID().",$servid,\"".IpLogger::getRealIP()."\",NOW())";
				if($con->query($sql)){
					return true;
				}else{
					return false;
			...
			if(self::checkView($servid,$field,$table)==true){
				$con = Database::getCon();
				$tim = time();
				$sql = "insert into $table ($field,realip,created_at) values ($servid,\"".IpLogger::getRealIP()."\",NOW())";
				if($con->query($sql)){
					return true;
				}else{
					return false;
